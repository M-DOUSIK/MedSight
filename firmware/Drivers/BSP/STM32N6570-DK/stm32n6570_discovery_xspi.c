...
/* A diagnostic, kept and gated OFF rather than deleted - the same
...
int32_t BSP_XSPI_NOR_Init(uint32_t Instance, BSP_XSPI_NOR_Init_t *Init)
{
  ...
  else
  {
    ...
    if (XSPI_Nor_Ctx[Instance].IsInitialized == XSPI_ACCESS_NONE)
    {
      ...
      {
        /* ── Accept a chip that is already in Octal-DTR ───────
         *
...
static int32_t XSPI_NOR_ResetMemory(uint32_t Instance)
{
   ...
   *
   * AN EARLIER NOTE SAID: "Development boot is unaffected: there the chip is in
   * SPI after power-on, this read gets nothing, and the original sequence
   * runs exactly as it always has." THAT WAS WRONG, AND IT WAS NEVER
   * TESTED - it was an assumption written as a reassurance.
   *
   * A probe that FAILS is not free. MX66UW1G45G_ReadStatusRegister() is
   * HAL_XSPI_Command() followed by HAL_XSPI_Receive(), both bounded by
   * HAL_XSPI_TIMEOUT_DEFAULT_VALUE. Asked in Octal-DTR of a chip that is
   * still in single-SPI - which is exactly development boot, where nothing
   * has configured the device - the receive times out and leaves a
   * transaction in flight with the peripheral's BUSY flag set. Every
   * HAL_XSPI_Command() after that returns HAL_BUSY, so the SPI status read
   * below fails, the reset commands fail, and ResetMemory reports failure
   * on a chip that was never even spoken to properly. All three retries in
   * npu_init.c then fail identically, because re-running HAL_XSPI_Init()
   * does not clear a stuck transaction.
   *
   * That is the mirror image of the fault this probe was added to fix, and
   * it is the same lesson as the XSPI2->CR read that hung development boot
   * once before: touching a peripheral to ask it a question is an
   * operation, not a glance. The abort below is the cost of asking. */
  ...
  /* The probe above timed out, so the peripheral is still holding that
   * transaction. Clear it before anything else is attempted. Abort is safe
   * either way: with BUSY set it issues XSPI_CR_ABORT and waits for the
   * device to go idle; with BUSY clear it only drops FMODE and returns the
   * handle to READY. Unreachable on the boot-from-flash path, which took
   * the early return above. */
  (void)HAL_XSPI_Abort(&hxspi_nor[Instance]);
  ...
  else
  {
    ...
    /* MedSight fix: wait for the reset to actually complete.
     *
     ...
     *
     * Same shape as the GT911 fix: never let a device
